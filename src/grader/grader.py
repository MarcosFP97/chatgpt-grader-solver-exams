...
if __name__ == '__main__':
	...
	notas_total = []
	if True == True:   #Uso esto para decidir entre ejecutarlo todo (todos los exámenes) o solo uno (de prueba)
	...
	if True == False:
		notas = cal_examen('exmayo2023.docx', examenes[1])
		notas['Examen'] = examenes[1]
		print(notas)
		nota_final = calculo_nota_final(notas)
		print(nota_final)
		...
		keys = [keys[-1]] + keys[:-1]  #Ponemos el nombre del examen al que correponden las notas al inicio
		notas = [notas]
		with open('calificaciones.csv', 'w') as calificaciones:
			writer = csv.DictWriter(calificaciones, fieldnames=keys)
			writer.writeheader()
